...
{{/if}}
{*值是否需要 数组类型 判断*}
{{$array_check = $item->type|php_array_check}}
{*变量类型*}
{{$item_type = $item->type|item_type_name}}
{*判断数组存在这个key 如果是list struct map，还需要判断这个key是否是数组*}
{{if $isset_check && $array_check}}
    {{$rank|indent}}if (isset(${{$data_value}}) && is_array(${{$data_value}})) {
    {{++$rank}}
    {{elseif $isset_check}}
    {{$rank|indent}}if (isset(${{$data_value}})) {
    {{++$rank}}
    {{elseif $array_check}}
    {*如果只需要判断 是不是数组，不是数组就continue了*}
    {{$rank|indent}}if (!is_array(${{$data_value}})) {
...
{{if $item->type|php_simple_type}}
    ...
    {{$rank|indent}}${{$var_name}} = {{$convert}}${{$data_value}};
{*对象*}
{{elseif 'struct' === $item_type}}
    {{$tmp_var_name = $key_name|tmp_var_name:'struct'}}
    {*这里多写4个空格，因为连续的 模板标签之间的空格会被忽略*}
    {{$rank|indent}}${{$tmp_var_name}} = new {{$item->struct_name}};
    {{$rank|indent}}${{$tmp_var_name}}->init(${{$data_value}});
    {{$rank|indent}}${{$var_name}} = ${{$tmp_var_name}};
{*枚举数组*}
{{elseif 'list' === $item_type}}
    ...
    {*list类型*}
    {{$sub_item = $item->item}}
    {{$rank|indent}}foreach (${{$data_value}} as ${{$for_var_name}}) {
    {*缩进*}
    {{++$rank}}
    {{php_item_init var_name=$for_var_name rank=$rank data_name=$for_var_name item=$sub_item}}
    {{$rank|indent}}${{$result_var_name}}[] = ${{$for_var_name}};
    {*退格*}
    {{$rank--}}
    {{$rank|indent}}}
    {{$rank|indent}}${{$var_name}} = ${{$result_var_name}};
{*关联数组*}
{{elseif 'map' === $item_type}}
    ...
    {{$rank|indent}}foreach (${{$data_value}} as ${{$key_var_name}} => ${{$for_var_name}}) {
    {*缩进*}
    {{++$rank}}
